...
def decompose(data_with_size: bytes):
    """
...
def concat(*xs: bytes) -> bytes: return compose(xs)
def concat_pk_str(pk: rsa.RSAPublicKey, bs: bytes) -> bytes: return compose([crypto.serialize_pkey(pk), bs])
def unconcat_pk_str(bs: bytes):
    xs = decompose(bs)
    if len(xs) != 2:
...
def stringbot_from(bs: bytes) -> bytes:
    if bs == b'':
        raise BadCall()
    elif bs[0] == b'N':
        return None
...
def stringbot_to(bs: bytes) -> bytes:
    if bs is None:
        return b'N'
    else:
        return b'S' + bs
